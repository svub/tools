...
_.extend u.l,
  ...
  serialize: (location) ->
    if not location? or location is false then '_'
      ...
      else "#{sanitize location.label};#{trim location.lat},#{trim location.lng},#{location.distance}"
  ...
  createFromOsmData: (data) ->
    ...
    l = u.l.create data.display_name, data.lat, data.lon, distance
    # l.tokens = u.removeAll(l.label, ',', ';')?.split(' ') ? []
